    ...
    return (
      <div className="flex h-full w-full flex-col items-center justify-center p-4">
        <div className="flex flex-col items-center gap-4 text-center">
            ...
            <p className="max-w-md text-muted-foreground">Please wait while we retrieve your session data.</p>
        </div>
      </div>
    );
  }
            ...
            {isAnalyzing || !analysis ? (
                 <CardContent className="text-center py-10">
                    <div className="flex flex-col items-center gap-4 text-center">
                        ...
                        <p className="max-w-md text-muted-foreground">Our AI is reviewing your transcript and generating detailed feedback. This page will update automatically when it's ready.</p>
                    </div>
                </CardContent>
            ) : (
            ...
            <CardContent className="space-y-4">
              {interview.transcript.map((message, index) => (
                 <div key={index} className={`flex items-start gap-3 ${message.role === 'user' ? 'justify-end' : ''}`}>
                    ...
                    <div className={`p-3 rounded-lg max-w-lg ${message.role === 'model' ? 'bg-muted' : 'bg-primary text-primary-foreground'}`}>
                        <p>{message.content}</p>
                    </div>
                     {message.role === 'user' && (
                        <div className="bg-muted/80 text-muted-foreground rounded-full p-2 shrink-0">
