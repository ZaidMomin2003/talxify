...
const LiveInterviewComponent = () => {
  ...
  const interviewId = (params?.interviewId as string) || `draft_${Date.now()}`;
  ...
  const startInterview = useCallback(async (stream: MediaStream) => {
  ...
  }, [isMuted, cleanupMedia, jobRole, company]);
   ...
   const finishInterviewAndSave = useCallback(async () => {
    ...
    // Don't save if it's a draft session
    if (params?.interviewId) {
        if (user && interviewId) {
            try {
                const activity: InterviewActivity = {
                    ...
                    details: { topic: searchParams.get('topic') || "Live Interview", role: jobRole, company }
                };
        ...
        toast({ title: "Draft Interview Complete!", description: "This was a draft session and was not saved." });
        setStatus('idle');
    }
  }, [cleanupMedia, user, interviewId, router, toast, searchParams, jobRole, company, params?.interviewId]);
  ...
  const toggleVideo = () => {
   ...
   const renderForm = () => (
    <div className="absolute inset-0 z-20 flex items-center justify-center bg-black/30 backdrop-blur-sm">
        <div className="w-full max-w-md p-8 bg-card/80 border rounded-lg shadow-xl text-center">
            ...
            <div className="space-y-4">
                ...
                <Button onClick={requestMediaAndStart} className="w-full" size="lg">Start Interview</Button>
            </div>
        </div>
    </div>
  );
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden bg-black text-white">
        ...
        {status !== 'idle' && renderInterviewControls()}
        {status !== 'idle' && renderMessages()}
    </div>
  );
};
