...
import { Github, Linkedin, Instagram, Mail, Phone, Link as LinkIcon, Award, Briefcase, MessageSquare, GraduationCap, Sparkles, Building, Calendar, Star, Code, Twitter, Globe, School, Percent, Loader2, Bot, User as UserIcon, BarChart, Youtube, HelpCircle, Menu, Eye } from "lucide-react";
                ...
                <div className="flex items-center justify-between h-16">
                    ...
                    <div className="flex items-center gap-4">
                        <div className="flex items-center space-x-2">
                            ...
                            <Label htmlFor="recruiter-mode" className="text-sm font-medium text-muted-foreground">Recruiter Mode</Label>
        ...
        <div className="space-y-8">
             <Card className="overflow-hidden shadow-lg">
                <div className="grid md:grid-cols-3 items-center">
                    <div className="md:col-span-1">
                        <Image src={portfolio.personalInfo.avatarUrl || "https://placehold.co/400x400.png"} alt={portfolio.personalInfo.name} width={400} height={400} className="w-full h-full object-cover" data-ai-hint="person portrait" />
                    </div>
                    <div className="md:col-span-2 p-8 space-y-4">
                        <h1 className="text-4xl font-bold font-headline">{portfolio.personalInfo.name}</h1>
                        <h2 className="text-2xl font-semibold text-primary">{portfolio.personalInfo.profession}</h2>
                        <p className="text-muted-foreground">{portfolio.personalInfo.bio}</p>
                        <div className="flex items-center gap-4 pt-2">
                            <Button variant="outline" asChild><a href={`mailto:${portfolio.personalInfo.email}`}><Mail className="mr-2"/>Email</a></Button>
             ...
             <div className="grid gap-6 sm:grid-cols-3">
                <Card>
                    ...
                    <CardContent><p className="text-4xl font-bold">{interviewsCompleted}</p><p className="text-sm text-muted-foreground">Completed</p></CardContent>
                </Card>
                <Card>
                    <CardHeader><CardTitle>Problems Solved</CardTitle></CardHeader>
                    <CardContent><p className="text-4xl font-bold">{questionsSolved}</p><p className="text-sm text-muted-foreground">in Coding Gym</p></CardContent>
                </Card>
                <Card>
                    <CardHeader><CardTitle>Average Score</CardTitle></CardHeader>
                    <CardContent><p className="text-4xl font-bold">{averageScore}%</p><p className="text-sm text-muted-foreground">Across all activities</p></CardContent>
                </Card>
            </div>
            ...
            {portfolio.projects.length > 0 && (
                <Card>
                    <CardHeader><CardTitle>Latest Project: {portfolio.projects[0].title}</CardTitle></CardHeader>
                    <CardContent>
                        {portfolio.projects[0].imageUrl && <Image src={portfolio.projects[0].imageUrl} alt={portfolio.projects[0].title} width={800} height={450} className="w-full rounded-lg mb-4" />}
                        <p className="text-muted-foreground mb-4">{portfolio.projects[0].description}</p>
                        <Button asChild><a href={portfolio.projects[0].link} target="_blank" rel="noopener noreferrer">View Project <LinkIcon className="ml-2"/></a></Button>
                    </CardContent>
                </Card>
            )}
            ...
            {portfolio.experience.length > 0 && (
                 <Card>
                    <CardHeader><CardTitle>Recent Experience</CardTitle></CardHeader>
                    <CardContent>
                         <p className="text-xl font-semibold">{portfolio.experience[0].role}</p>
                        <p className="text-muted-foreground">{portfolio.experience[0].company} &bull; {portfolio.experience[0].duration}</p>
                    </CardContent>
                </Card>
            )}
        </div>
